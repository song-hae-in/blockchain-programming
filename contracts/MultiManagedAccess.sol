...
contract MultiManagedAccess {
    uint constant MANGERS_NUMBERS = 5;
    uint immutable BACKUP_MANAGER_NUMBERS;
    address public owner;
    address[MANGERS_NUMBERS] public managers;
    bool[MANGERS_NUMBERS] public confirmed;

    constructor(
        address _owner,
        address[MANGERS_NUMBERS] memory _managers,
        uint _managers_numbers
    ) {
        require(_managers.length == _managers_numbers, "size unmatched");
        owner = _owner;
        BACKUP_MANAGER_NUMBERS = _managers_numbers;
        for (uint i = 0; i < MANGERS_NUMBERS; i++) {
